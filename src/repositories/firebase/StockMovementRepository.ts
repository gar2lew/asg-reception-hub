...
export class FirebaseStockMovementRepository {
  async getAll(): Promise<StockMovement[]> {
    const q = query(collection(getFirestoreDb(), COLL), orderBy('createdAt', 'desc'), limit(50));
    const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() } as StockMovement));
  }
  async getByOffice(office: string): Promise<StockMovement[]> {
    const q = query(collection(getFirestoreDb(), COLL), where('office', '==', office), orderBy('createdAt', 'desc'), limit(50));
    const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() } as StockMovement));
  }
